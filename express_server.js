...
const usersDatabase = require('./userDatabase')
...
app.get("/urls", (req, res) => {
  ...
  if (!userID) {
    res.status(401).send('<h1>You must be <a href="/login">logged in</a> to view this page!</h1>');
  } else {
...
app.post("/urls", (req, res) => {
  if (req.session.user_id) {
    ...
    res.status(401).send('<h1>You must be <a href="/login">logged in</a> to create new tiny urls!</h1>');
  }
});
...
app.post("/login", (req, res)=> {
  const foundUserEmail = helperFunction.getUserEmail(req.body.email, usersDatabase);
  if (foundUserEmail) {
...
app.get("/urls/:shortURL", (req, res) => {
  const userID = req.session.user_id;
  if (!userID) {
    res.status(401).send('<h1>You must be <a href="/login">logged in</a> and the creator to view this page!</h1>');
  } else {
    ...
    if (shortURL) {
      if (urlDatabase[shortURL] === undefined) {
        ...
        res.status(401).send('<h1>You must be the creator to view this page! Return to <a href="/urls">url</a> page.</h1>');
      }else {
        const templateVars = {
          shortURL: shortURL,
...
app.get("/u/:shortURL", (req, res) => {
  if (!urlDatabase[req.params.shortURL]) {
    res.status(404).send('<h1>URL does not exist. Please return to the <a href="/urls">URL</a> Page!</h1>');
  } else {
